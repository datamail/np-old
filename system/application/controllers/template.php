...
void textblock(String textblockstring, int textblockx, int textblocky, int textblockwidth)
{
	...
	for(int a = 0; a < textblock.length; a++)
	{
		...
		elseif ($this->form_validation->run() == TRUE){
			...
			//$auto_setup_code = '/* @pjs preload="/template_images/121.png"; */' .
...
void textblock(String textblockstring, int textblockx, int textblocky, int textblockwidth)
{
	...
	for(int a = 0; a < textblock.length; a++)
	{
		...
		text(textblock[a]+ " ", textblockw + textblockx - textWidth(textblock[a] + " "), textblockynow);
			...
			$query_str = "UPDATE templates SET type = '$template_type', discrete_inputs = '$template_number_discrete', continuous_inputs = '$template_number_continuous', notes = '$template_notes', last_edited = '$now', flagged = '0' WHERE template_id = '$template_id'"; 
	...
	function search(){
		...
		if ($this->form_validation->run() == FALSE && $this->session->userdata('doing_search') != TRUE) {
			...
			if($config['total_rows'] > 0)
			{
				...
				$query = $this->db->query("SELECT * FROM templates WHERE flagged = '0' ORDER BY last_edited DESC");
			...
			$full_query = "SELECT * FROM templates WHERE " . $type_query . $continuous_query . $discrete_query . " AND flagged = '0' ORDER BY last_edited " . $orderby  . " LIMIT " . $limit . ", " . $offset;	

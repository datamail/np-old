...
for(int a =1; a < data.length; a++)
{
rect(100 + a * column_width_with_space, 120, column_width, int(data[a][1]) * multiplier);
rotate(PI/2);
translate(0, -height);
text(data[a][0] , 130 +  int(data[a][1]) * multiplier, 900 - a * column_width_with_space  - (column_width_with_space / 2)  );
translate(0, height);
rotate(-PI/2);
}

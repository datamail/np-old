...
{
 height_counter = 0;
 while(height_counter < data[a][3] * height_multiplier)
 {
 ...
 if (data[a][3] == -1)
  {
  text("? ? ? ? ?", 750, 900 - (column_width_with_space / 2 + column_width_with_space * (a - 1)));
  };
 if (data[a][3] == 3 && data [a][2] == "Code")
  {
  text("+ +", 590, 900 - (column_width_with_space / 2 + column_width_with_space * (a - 1)));
  };
 translate(0, height);
 rotate(-PI/2);
